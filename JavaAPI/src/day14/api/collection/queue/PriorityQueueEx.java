...
//		PriorityQueue<Integer> queue = new PriorityQueue<>();
		...
		//queue가 비웠는지 확인
		if(queue.isEmpty() == false) {//queue가 비워져있으면 true, 아니면 false
			System.out.println("큐가 비워있지 않음");
		}
		System.out.println("-------------------------------------");
		/*UserVO를 담을 수 있는 Queue
		큐가 순서를 확인할 때 compareTo메서드를 확인합니다.
		객체를 우선순위큐에 저장할 때는 compareTo에 정의된 형식에 따라서 우선순위방법을 지정해줘야 합니다
		compable인터페이스를 구현해서 compareTo메서드를 오버라이딩하면 됩니다.
		*/
		
		System.out.println("홍길동".compareTo("홍길자")); // 음수가 나오면 홍길자가 사전적으로 뒤에 위치
		System.out.println("홍길자".compareTo("홍길동")); // 양수가 나오면 홍길자가 사전적으로 앞에 위치
